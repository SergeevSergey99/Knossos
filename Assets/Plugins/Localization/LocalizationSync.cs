...
public class LocalizationSync : MonoBehaviour
{
    ...
    public UnityEngine.Object SpamFolder;
    ...
    public void Spawn()
    {
        var textAssets = Resources.LoadAll<TextAsset>("LocalizationCSV");
        var folder = UnityEditor.AssetDatabase.GetAssetPath(SpamFolder);
        ...
        foreach (var textAsset in textAssets)
        {
            ...
            for (var i = 1; i < lines.Count; i++)
            {
...

                {
                    ...
                    asset.EN = columns[1];
                    asset.RU = columns[2];
                    Debug.Log("Creating asset: " + folder + "/" + textAsset.name + "/" + key + ".asset");
                    if (!AssetDatabase.IsValidFolder(folder + "/" + textAsset.name))
                        AssetDatabase.CreateFolder(folder, textAsset.name);
                    AssetDatabase.CreateAsset(asset, folder + "/" + textAsset.name + "/" + key + ".asset");
                }
            }
